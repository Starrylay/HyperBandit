...
class LinUCBUserStruct:
    ...
    def updateParameters(self, articlePicked_FeatureVector, click):
        ...
        self.A = 1.0*self.A + np.outer(articlePicked_FeatureVector,articlePicked_FeatureVector)
        self.b = 1.0*self.b + articlePicked_FeatureVector*click
        self.AInv = np.linalg.inv(self.A)
...
class Hybrid_LinUCB_singleUserStruct(LinUCBUserStruct):
    ...
    def updateParameters(self, articlePicked_FeatureVector, click):
        ...
        self.B = 1.0 * self.B + np.outer(articlePicked_FeatureVector, additionalFeatureVector)
...
class Hybrid_LinUCBUserStruct:
    ...
    def updateParameters(self, articlePicked_FeatureVector, click, userID):
        ...
        self.A_z = 1.0*self.A_z + np.dot(temp, self.users[userID].B)
        self.b_z = 1.0*self.b_z + np.dot(temp, self.users[userID].b)
...
class Hybrid_LinUCBAlgorithm:
    ...
    def __init__(self,setting):
        self.dimension = setting.user_dimension
        self.alpha = setting.alpha
        self.lambda_ = setting.lambda_
        self.dataset = setting.dataset
        self.feature = setting.feature
        UserFeatureVectors, _, _ ,_= util.read_in_observed_feature(self.dataset, self.feature)
        self.USER = Hybrid_LinUCBUserStruct(self.dimension , self.lambda_, UserFeatureVectors)
   
    def update_user(self, userID, userFeature):
        self.USER.users[userID] = Hybrid_LinUCB_singleUserStruct(userFeature[:self.dimension], self.lambda_ , userID)
