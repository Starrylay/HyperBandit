...
class LinUCBUserStruct:
    ...
    def updateParameters(self, articlePicked_FeatureVector, click):
        self.A =  self.A + np.outer(articlePicked_FeatureVector, articlePicked_FeatureVector)
        self.b =  self.b + articlePicked_FeatureVector * click
        self.AInv = np.linalg.inv(self.A)
...
class SlaveLinUCBStruct(LinUCBUserStruct):
    ...
    def updateParameters(self, articlePicked_FeatureVector, click):
        ...
        self.A = 0.95* self.A + np.outer(articlePicked_FeatureVector, articlePicked_FeatureVector)
        self.b = 0.95* self.b + articlePicked_FeatureVector * click
        self.AInv = np.linalg.inv(self.A)
        self.UserTheta = np.dot(self.AInv, self.b)
        self.time += 1
...
class DLinUCBAlgorithm:
    def __init__(self,setting):  # n is number of users #, dimension, alpha, lambda_, NoiseScale, tau, delta_1=1e-2, delta_2=1e-1, tilde_delta_1=1e-2,eta=0.3
        self.dimension = setting.item_dimension
        self.alpha = setting.alpha
        self.lambda_ = setting.lambda_
        self.global_time = 0
        self.users = {}
        self.NoiseScale = setting.NoiseScale
        self.ObservationInterval = setting.tau  # Size of the slding window
        self.delta_1 = setting.delta_1
        self.delta_2 = setting.delta_2
        self.tilde_delta_1 = setting.tilde_delta_1  # self.tilde_delta_1 is between 0 and self.delta_1
        self.eta = setting.eta  # self.eta is between 0 and 1
        self.CanEstimateUserPreference = True
        self.CanEstimateUserCluster = False

#self, pool_items_info, user_info, Theta
    def decide(self, pool_articles, user_info, Theta):
        userID = user_info.id
        if userID not in self.users:
            self.users[userID] = MASTER(self.global_time)
            self.users[userID].SLAVEs.append(
                SlaveLinUCBStruct(featureDimension=self.dimension, alpha=self.alpha, lambda_=self.lambda_,
                                  createTime=self.global_time, NoiseScale=self.NoiseScale, delta_1=self.delta_1,
...
# self, item_info_picked, reward, user_info, Theta
    def updateParameters(self, articlePicked, click, user_info, Theta):
        ...
        for alg in self.users[userID].SLAVEs:
            ...
            data = alg.getSlavePredictionInfo(alg.alpha, articlePicked.contextFeatureVector[:self.dimension])
        ...
        if CreateNewFlag or len(self.users[userID].SLAVEs) == 0:
            # print("==========================create model")
            self.users[userID].newUCBs.append(self.users[userID].time)
            new_ucb = SlaveLinUCBStruct(featureDimension=self.dimension, alpha=self.alpha, lambda_=self.lambda_,
                                        createTime=self.users[userID].time, NoiseScale=self.NoiseScale,
        ...
        for alg in self.users[userID].SLAVEs:
            if alg in good_alg:
                alg.updateParameters(articlePicked.contextFeatureVector[:self.dimension], click)
                alg.updates += 1
