...
class BaseUserStruct:
    ...
    def updateParameters(self, item_info_picked, reward):
        ...
        self.A = 0.99 * self.A + np.outer(articlePicked_FeatureVector, articlePicked_FeatureVector)
        self.b = 0.99 * self.b + articlePicked_FeatureVector * reward
        self.AInv = np.linalg.inv(self.A)
...
class LinUCBAlgorithm_UserBased:
    def __init__(self, setting):
        self.user_num = setting.user_num
        self.user_dim = setting.user_dimension
        self.item_dim = setting.item_dimension
