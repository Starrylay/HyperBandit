...
class CoLinUCBUserSharedStruct(object):
	...
	def updateParameters(self, articlePicked, click,  userID, update='Inv'):
		X = vectorize(np.outer(articlePicked.contextFeatureVector[:self.featureDimension], self.W.T[userID]))
		#print "X: " + str(X)
		change = np.outer(X, X)
		self.A += change
		self.b += click*X
		...
		self.UserTheta = matrixize(np.dot(self.AInv, self.b), len(articlePicked.contextFeatureVector[:self.featureDimension]))
		self.CoTheta = np.dot(self.UserTheta, self.W)
		self.CCA = np.dot(np.dot(self.BigW, self.AInv), np.transpose(self.BigW))
	
	def getProb(self, alpha, article, userID):
		warnings.filterwarnings('error')# 将任何警告转化为error终止程序
		TempFeatureM = np.zeros(shape =(len(article.contextFeatureVector[:self.featureDimension]), self.userNum))
		TempFeatureM.T[userID] = article.contextFeatureVector[:self.featureDimension]
		TempFeatureV = vectorize(TempFeatureM)
		
		mean = np.dot(self.CoTheta.T[userID], article.contextFeatureVector[:self.featureDimension])
...
class CoLinUCBAlgorithm(BaseAlg):
	...
	def decide(self, pool_articles, userID, k = 1):
		# MEAN
		art_features = np.empty([len(pool_articles), len(pool_articles[0].contextFeatureVector[:self.arg_dict['dimension']])])
		for i in range(len(pool_articles)):
			art_features[i, :] = pool_articles[i].contextFeatureVector[:self.arg_dict['dimension']]
		user_features = self.USERS.CoTheta.T[userID]
		mean_matrix = np.dot(art_features, user_features)

		# VARIANCE
		art_temp_features = np.empty([len(pool_articles), len(pool_articles[0].contextFeatureVector[:self.arg_dict['dimension']])*self.n_users])
		for i in range(len(pool_articles)):
			TempFeatureM = np.zeros(shape =(len(pool_articles[0].contextFeatureVector[:self.arg_dict['dimension']]), self.n_users))
			TempFeatureM.T[userID] = pool_articles[i].contextFeatureVector[:self.arg_dict['dimension']]
