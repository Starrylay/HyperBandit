...
class LinUCBUserStruct:
    ...
    #         np.log(np.linalg.det(self.A) / float(self.sigma * self.lambda_))) + np.sqrt(self.lambda_)
    #     # pta = mean + alpha * var
...
class dLinUCB:
    def __init__(self, dimension, alpha, lambda_, NoiseScale, tau, delta_1=1e-2, delta_2=1e-1, tilde_delta_1=1e-2,
                 eta=0.3):  # n is number of users
    ...
    def updateParameters(self, articlePicked, click, userID):
        ...
        for alg in self.users[userID].SLAVEs:
            data = alg.getSlavePredictionInfo(alg.alpha_t, articlePicked.featureVector)
